...
class DPD_Pricing {
	...
	public static function filter_price($price, $product) {
		...
		error_log('DPD Cart Filter: Product ID ' . $product_id . ', Price: ' . $price);
		...
		error_log('DPD Cart Filter: Selected context: ' . print_r($selected, true));
		
		if (!$selected) { 
			error_log('DPD Cart Filter: No selected context, returning original price');
			return $price; 
		}
		...
		if ($rule) { 
			...
			error_log('DPD Cart Filter: Applied rule, adjusted price from ' . $price . ' to ' . $adjusted);
			return $adjusted;
		}
		
		error_log('DPD Cart Filter: No rule found, returning original price');
		return $price;
	}
	...
	protected static function get_selected_datetime_context(int $product_id): ?array {
		...
		$use_ts = $ts;
		error_log('DPD Cart Context: Looking for product ID ' . $product_id);
		
		if (function_exists('WC') && WC()->cart) {
			$cart_items = WC()->cart->get_cart();
			error_log('DPD Cart Context: Cart has ' . count($cart_items) . ' items');
			
			foreach ($cart_items as $cart_item_key => $cart_item) {
				error_log('DPD Cart Context: Checking cart item - product_id: ' . ($cart_item['product_id'] ?? 'none') . ', variation_id: ' . ($cart_item['variation_id'] ?? 'none') . ', datetime: ' . ($cart_item[DPD_Frontend::FIELD_KEY] ?? 'none'));
				...
				if ($matches && !empty($cart_item[DPD_Frontend::FIELD_KEY])) {
					...
					if ($dt instanceof DateTimeImmutable) { 
						...
						error_log('DPD Cart Context: Using datetime timestamp: ' . $use_ts);
						break; 
					}
				}
			}
		} else {
			error_log('DPD Cart Context: No cart available');
		}
		...
		error_log('DPD Cart Context: Final context: ' . print_r($context, true));
		return $context;
	}
	...
	public static function filter_cart_item_price($price_html, $cart_item, $cart_item_key) {
		error_log('DPD Cart Item Price: Called for cart item key ' . $cart_item_key);
		
		if (empty($cart_item[DPD_Frontend::FIELD_KEY])) {
			error_log('DPD Cart Item Price: No datetime data, returning original price');
			return $price_html;
		}
		...
		$product = wc_get_product($product_id);
		if (!$product) {
			error_log('DPD Cart Item Price: Product not found for ID ' . $product_id);
			return $price_html;
		}
		...
		$dt = date_create_immutable_from_format('Y-m-d\TH:i', $val, wp_timezone());
		if (!$dt) {
			error_log('DPD Cart Item Price: Invalid datetime format: ' . $val);
			return $price_html;
		}
		...
		error_log('DPD Cart Item Price: Context: ' . print_r($ctx, true));
		...
		if ($rule) {
			...
			error_log('DPD Cart Item Price: Applied rule, adjusted from ' . $original_price . ' to ' . $adjusted_price);
			return wc_price($adjusted_price);
		}
		
		error_log('DPD Cart Item Price: No rule found, returning original price');
		return $price_html;
	}
	
	public static function filter_cart_item_subtotal($subtotal_html, $cart_item, $cart_item_key) {
		error_log('DPD Cart Item Subtotal: Called for cart item key ' . $cart_item_key);
		...
		if ($rule) {
			...
			error_log('DPD Cart Item Subtotal: Adjusted subtotal from ' . ($original_price * $quantity) . ' to ' . $adjusted_subtotal);
	...
	public static function apply_cart_item_pricing($cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data) {
		error_log('DPD Add to Cart: Item added with key ' . $cart_item_key . ', product_id ' . $product_id . ', variation_id ' . $variation_id);
		error_log('DPD Add to Cart: Cart item data: ' . print_r($cart_item_data, true));
		...
		if (!empty($cart_item_data[DPD_Frontend::FIELD_KEY])) {
			error_log('DPD Add to Cart: Datetime data found, item should be priced correctly');
		}
	}
}
